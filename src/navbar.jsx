...
export function Navbar() {
  ...
  return (
  <>
    <nav className="fixed top-4 left-1/2 transform -translate-x-1/2 w-10/12 lg:w-4/5 bg-gray-900/90 backdrop-blur-sm rounded-xl z-30 border border-teal-400/10 shadow-2xl shadow-teal-400/20">
      <div className="flex items-center justify-between px-3 lg:px-6 py-3">
        ...
        <div className="hidden lg:flex items-center justify-end absolute left-1/2 transform -translate-x-1/2 space-x-1">
          {navItems.map((item) => (
            item.type === "page" ? (
              <Link
                key={item.id}
              ...
              <button
                ...
                className="px-4 py-2 text-gray-300 whitespace-nowrap hover:text-teal-400 hover:bg-teal-400/10 rounded-lg transition-all duration-250 ease-in-out hover:-translate-y-0.5 font-medium text-sm"
              >
                {item.label}
              </button>
            )
        ...
        {/* Spacer for desktop to balance the logo on left */}
